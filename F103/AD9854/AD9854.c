...
#include "AD9854_fre_to_out.h"
#include "AD9854_fre_to_out_terminate.h"
#include "AD9854_fre_to_out_initialize.h"
...
void AD9854_IO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF|RCC_APB2Periph_GPIOC, ENABLE);	 //使能PB,PE端口时钟
...
//说明：   该算法为多字节相乘算法，有公式FTW = (Desired Output Frequency × 2N)/SYSCLK
...
//====================================================================================
//函数名称:void AD9854_SetSine_Fitted(ulong Freq,uint Shape)
//函数功能:AD9854正弦波产生程序,matlab拟合
//入口参数:Freq   频率设置，取值范围为0~(1/2)*SYSCLK
//         Shape  幅度设置. 为12 Bit,取值范围为(0~4095) ,取值越大,幅度越大。每Shape_perV输出峰峰值1v
//出口参数:无
//====================================================================================
#define Shape_perV 2000
double AD9854_fre_to_out_J4(double fre)
{
	double range;
	if(fre>30000000)
		range=4095;
...
double AD9854_fre_to_out_J2(double fre)
{
	double range;
	if(fre>30000000)
		range=4095;
...
void AD9854_SetSine_Fitted(ulong Freq,uint Shape)
{
	uchar count;
	uchar Adress;
	uint Shape_Q;
	...
	Shape_I=Shape*AD9854_fre_to_out_J4(Freq)/Shape_perV;
//	
//	printf(num2str(Shape_I));
//	printf("\r\n");
	
	AD9854_WR_Byte(0x21,Shape_I>>8);	  //设置I通道幅度
	AD9854_WR_Byte(0x22,(Shape_I&0xff));
	
	Shape_Q=Shape*AD9854_fre_to_out_J2(Freq)/Shape_perV;
	
	AD9854_WR_Byte(0x23,(Shape_Q*4/5)>>8);	  //设置Q通道幅度
	AD9854_WR_Byte(0x24,((Shape_Q*4/5)&0xff));//TODO
