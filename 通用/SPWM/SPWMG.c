...
{  
    ...
    TIM_TimeBaseInit(TIM1, &TIM_TimeBaseStructure);                     //初始化定时器配置

    /* Channel_1   TIM_OCMode_PWM1模式 */  
    TIM_OCInitStructure.TIM_OCMode       = TIM_OCMode_PWM1;             //向上计数时,一旦TIMx_CNT<TIMx_CCR1时通道1为有效电平
    ...
    TIM_OC2Init(TIM1, &TIM_OCInitStructure);                            //根据指定参数初始化外设TIM1 OC1
    ...
    TIM_OCInitStructure.TIM_OCIdleState  = TIM_OCIdleState_Set;         //输出空闲状态
    TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCNIdleState_Reset;      //PWM互补输出空闲状态
    TIM_OC3Init(TIM1, &TIM_OCInitStructure);                            //根据指定参数初始化外设TIM1 OC2

    TIM_OC2PreloadConfig(TIM1, TIM_OCPreload_Enable);                   //使能TIM3在CCR1的预装载寄存器
    TIM_OC3PreloadConfig(TIM1, TIM_OCPreload_Enable);                   //使能TIM3在CCR2的预装载寄存器
    ...
    TIM_CtrlPWMOutputs(TIM1,ENABLE);                                    //PWM输出使能(输出PWM的这句一定要加，有时必须加在主函数里，因为在这个子函数里可能不会有效果)
}

/**************************************************
*   名称:利用定时器2产生中断改变SPWM波的占空比  
***************************************************/
void TIM2_Int_Init(u16 ifreq,u16 psc)                       //ifreq为 不分频时的更新中断频率,psc为预分频值
{
    TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
    ...
    NVIC_InitStructure.NVIC_IRQChannelSubPriority        = 4;           //从优先级为4级
