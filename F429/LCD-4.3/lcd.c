...
//x,y:坐标
//POINT_COLOR:此点的颜色
void LCD_DrawPoint(u16 x,u16 y,u16 Colour)
{
...
//(x,y)表格的起始坐标
//(l,w)表格一个格子的长宽
//(x1,y1)表格的个数(x1表示横向，y1表示纵向)
void LCD_Paintingform(u16 x,u16 y,u16 l,u16 w,u16 x1,u16 y1,u16 Colour)//起始坐标,格子的长宽,表格的个数(x1表示横向，y1表示纵向)
{
	int i;
	for(i=0;i<x1+1;i++)//画纵线
	{
		LCD_DrawLine(x,y,x,y+y1*w,Colour);
		x=x+l;
	}
	for(i=0;i<y1+1;i++)//画横线
	{
		LCD_DrawLine(x-(x1+1)*l,y,x-l,y,Colour);
		y=y+w;
	}
}
...
//画矩形	  
//(x1,y1),(x2,y2):矩形的对角坐标
void LCD_DrawRectangle(u16 x1, u16 y1, u16 x2, u16 y2, u16 Colour)
{
...
//(x,y):中心点
//r    :半径
void LCD_Draw_Circle(u16 x0,u16 y0,u8 r, u16 Colour)
{
	int a,b;
	int di;
	a=0;b=r;	  
...
//Colour :颜色
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 Colour)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	...
	{ 
		...
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,Colour);
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size,u16 Colour)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0,Colour);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0,Colour); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode,u16 Colour)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar(x+(size/2)*t,y,'0',size,mode&0X01,Colour);  
				else LCD_ShowChar(x+(size/2)*t,y,' ',size,mode&0X01,Colour);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,mode&0X01,Colour); 
	}
} 
//显示字符串
//x,y:起点坐标
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u8 mode,u16 Colour)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,mode&0X01,Colour);
        x+=size/2;
        p++;
    }  
}
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowString2(u16 x,u16 y,u16 width,u16 height,u8 size,const u8 *p,u8 mode,u16 Colour)
{         
	u8 x0=x;
	width+=x;
	height+=y;
...
void LCD_Showchinese(u16 x,u16 y,u16 width,u16 height,u8 num,u8 size,u8 mode,u16 Colour)
{
	u8 temp,t1,t;
	u16 y0=y;
...
// 	num=num-' ';//
	for(t=0;t<csize;t++)
	{ 
		if (size==16)temp=asc1_1616[num][t]; 	 	//调用1616字体
		else if(size==24)temp=asc1_2424[num][t];	//调用2424字体
		else if(size==36)temp=asc1_3636[num][t];	//调用3636字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,Colour);
...
//使用用例
/*
LCD_ShowString(30,10,80,1,12,"STM32F407ZGT6",1,RED);	
		LCD_ShowString(30,30,200,1,16,"STM32F407ZGT6",1,RED);
		LCD_ShowString(30,50,200,2,24,"STM32F407ZGT6",1,RED);
 		LCD_ShowString(30,80,200,3,32,"STM32F407ZGT6",1,RED);		//显示字符串				 
		LCD_ShowString(30,120,300,4,48,"STM32F407ZGT6",1,RED);
		
		LCD_Showchinese(30,180 ,400,1,0,24,1,RED);
		LCD_Showchinese(55,180 ,400,1,1,36,1,RED);
		
		LCD_Showchinese(95,180 ,400,2,2,16,1,RED);//显示汉字
//		LCD_Showchinese(78,180 ,400,3,3,16,1,RED);
//		LCD_Showchinese(94,180 ,400,4,4,16,1,RED);
//    LCD_Showchinese(110,180 ,400,5,5,16,1,RED);
    LCD_ShowxNum(30,220,1234657890,11,12,1,BLACK);	//显示数字
    LCD_DrawRectangle(0,0,479,799,RED);//画矩形	
    LCD_DrawLine(0,0,479,799,YELLOW);	//直线
		LCD_DrawLine(0,799,479,0,YELLOW);
		LCD_Draw_Circle(238,399,50,RED);//圆
		LCD_Paintingform(100,100,20,20,15,15,RED);//表格
